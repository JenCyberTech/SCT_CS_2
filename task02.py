...
def open_img():
    ...
    if x:
        img = Image.open(x)
        eimg = img
        # Resize the image to fit within the designated area (for example, 300x300)
        ...
        if panelA is None or panelB is None:
            ...
            panelA.image = img
            panelA.place(x=100, y=300)  # Position the image in the designated area
            panelB = Label(image=img)
            panelB.image = img
            panelB.place(x=700, y=300)  # Position the image in the designated area
        else:
...
def en_fun(x):
    ...
    if image_input is not None:
        ...
        # Load and resize the encrypted image
        imge = Image.open('image_encrypted.jpg').resize((300, 300), Image.LANCZOS)
...
def de_fun():
    ...
    if image_encrypted is not None and key is not None:
        ...
        # Load and resize the decrypted image
        imgd = Image.open('image_output.jpg').resize((300, 300), Image.LANCZOS)
...
start1 = tk.Label(text="Original Image", font=("Arial", 40), fg="magenta")
start1.place(x=70, y=230)

start1 = tk.Label(text="Encrypted Decrypted Image", font=("Arial", 40), fg="magenta")
start1.place(x=580, y=230)
...
down_encrypted.place(x=320, y=720)
...
down_decrypted.place(x=540, y=720)
